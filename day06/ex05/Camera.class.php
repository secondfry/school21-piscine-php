...
class Camera {
  ...
  public function _checkWidthHeight() {
    ...
    $top = tan($this->_fov / 2) * $this->_near;
    $bottom = -$top;
    $right = $top * $this->_ratio;
    $left = -$right;
  ...
  public function watchTriangle(Triangle $worldTriangle): Triangle {
    return new Triangle(
      $this->watchVertex($worldTriangle->getA()),
      $this->watchVertex($worldTriangle->getB()),
      $this->watchVertex($worldTriangle->getC())
    );
  }

  public function watchMesh(array $triangles): array {
    $ret = [];
    foreach ($triangles as $triangle) {
      $ret[] = $this->watchTriangle($triangle);
    }
    return $ret;
  }
